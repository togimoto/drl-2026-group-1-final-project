...
class GRPOConfig(_BaseConfig):
    ...
    Parameters:
        ...
        positive_advantage_only (`bool`, *optional*, defaults to `False`):
            If `True`, zero out the per-token loss for any rollout whose advantage is `<= 0`, so only positive-advantage
            rollouts contribute gradient. Pilot experiment: tests whether negative-advantage gradients are cancelled by
            positive ones in the same batch. Mirrors the masking pattern used by `off_policy_mask_threshold`.
        positive_advantage_only_keep_kl (`bool`, *optional*, defaults to `False`):
            Only relevant when `positive_advantage_only=True` and `beta != 0`. If `False` (default), the
            positive-advantage mask is applied *after* the KL term is added, so non-positive-advantage rollouts are
            fully dropped (no policy-gradient AND no KL gradient). If `True`, the mask is applied *before* the KL
            term (matching the `off_policy_mask_threshold` precedent), so masked rollouts still contribute KL
            gradient.
        positive_advantage_only_steps_fraction (`float`, *optional*, defaults to `1.0`):
            Fraction of total training steps during which `positive_advantage_only` is active. Default `1.0` means
            always on. Set to e.g. `0.2` to apply positive-advantage masking only for the first 20% of training as
            a curriculum warmup and then fall back to vanilla GRPO. Only relevant when `positive_advantage_only=True`.
        negative_advantage_only (`bool`, *optional*, defaults to `False`):
            If `True`, zero out the per-token loss for any rollout whose advantage is `>= 0` during the final
            `negative_advantage_only_steps_fraction` of training, so only negative-advantage rollouts contribute
            gradient. Mirror of `positive_advantage_only`. Intended as a cooldown phase: late in training the model
            mostly succeeds, so failures carry the largest negative advantage and are the most informative signal.
            Combine with `positive_advantage_only=True` and `positive_advantage_only_steps_fraction=0.2` for a full
            3-phase curriculum: positive-warmup → full GRPO → negative-cooldown.
        negative_advantage_only_steps_fraction (`float`, *optional*, defaults to `0.2`):
            Fraction of total training steps (counted from the **end**) during which `negative_advantage_only` is
            active. Default `0.2` means the final 20% of training. Only relevant when `negative_advantage_only=True`.
            If both `positive_advantage_only` and `negative_advantage_only` are enabled, their fractions must not
            overlap (`positive_advantage_only_steps_fraction + negative_advantage_only_steps_fraction <= 1.0`).
        negative_advantage_only_keep_kl (`bool`, *optional*, defaults to `False`):
            Mirror of `positive_advantage_only_keep_kl` for the negative-cooldown phase. Only relevant when
            `negative_advantage_only=True` and `beta != 0`. If `False` (default), the negative-advantage mask is
            applied *after* the KL term, so non-negative-advantage rollouts are fully dropped (no policy-gradient
            AND no KL gradient). If `True`, the mask is applied *before* the KL term, so masked rollouts still
            contribute KL gradient. Has no effect when `beta=0`.
        linear_advantage_interpolation (`bool`, *optional*, defaults to `False`):
            If `True`, replace the binary positive/negative advantage masks with a per-step linear interpolation
            between two weights `pos_weight` and `neg_weight`. At step 0 the schedule is `(pos_weight, neg_weight)
            = (1.0, 0.0)` (positives-only); at the final step it is `(0.0, 1.0)` (negatives-only); intermediate
            steps are weighted linearly. When `positive_advantage_only=True` /
            `negative_advantage_only=True` are set, their `_steps_fraction` windows clamp the schedule endpoints,
            so the schedule stays at `(1.0, 0.0)` during the positive window and `(0.0, 1.0)` during the negative
            window, interpolating only in between. Incompatible with `reward_as_advantage=True`,
            `positive_advantage_only_keep_kl=True`, and `negative_advantage_only_keep_kl=True`.
        reward_as_advantage (`bool`, *optional*, defaults to `False`):
            If `True`, skip the group-relative baseline and use the (weighted-sum) reward directly as the advantage.
            Intended for binary `0/1` reward schemes. When `True`, `num_generations` may be set to `1` (REINFORCE-style
            single-rollout training); combine with `positive_advantage_only=True` to drop zero-reward rollouts.
        reward_as_advantage_signed (`bool`, *optional*, defaults to `False`):
            If `True` (requires `reward_as_advantage=True`), remap the binary reward via `advantages = 2 * rewards - 1`
            so `r=0` rollouts become `-1` instead of `0`. Under vanilla `reward_as_advantage`, failures contribute zero
            gradient; the signed variant additionally pushes the policy away from `r=0` rollouts without going through
            a group baseline. Foot-gun: combining with `positive_advantage_only=True` (always-on) silently masks out
            the new negatives — leave the mask off, or set `positive_advantage_only_steps_fraction < 1.0`.
        use_lens (`bool`, *optional*, defaults to `False`):
            If `True`, enable Likelihood Estimation with Negative Samples (LENS, Feng et al., 2025). Replaces the
            standard group-relative advantage with a confidence-calibrated reward that assigns non-zero penalties to
            incorrect generations based on the old policy's length-normalized sequence probability, so all-negative
            groups (zero-advantage under vanilla GRPO) contribute useful gradient. Requires binary `0/1` rewards and
            `multi_objective_aggregation="sum_then_normalize"`; incompatible with `reward_as_advantage`. Combines
            with `positive_advantage_only=True` only when `positive_advantage_only_steps_fraction < 1.0`
            (positive-only warmup, then LENS).
        lens_alpha (`float`, *optional*, defaults to `0.25`):
            Down-weighting factor applied to advantages of all-negative groups under LENS; paper sets this to `0.25`
            for all experiments. Ignored when `use_lens=False`.
        lens_apply_to_positives (`bool`, *optional*, defaults to `False`):
            If `True` (requires `use_lens=True`), also discount correct (`r=1`) rollouts by the same
            confidence-calibrated penalty LENS applies to incorrect rollouts: `r̃_i = r_i - (1/G) * π̄_old /
            (D - π̄_old)`. Rescues all-positive groups (zero-advantage under vanilla GRPO) by pushing the policy
            toward less-confident correct rollouts. Composes with `positive_advantage_only=True` to run a
            positives-only LENS pilot; in that combo, zero-reward rollouts are fully ignored.
        lens_apply_to_negatives (`bool`, *optional*, defaults to `True`):
            If `True` (default, requires `use_lens=True`), apply the LENS penalty to incorrect (`r=0`) rollouts —
            the standard paper behavior. Set to `False` to disable the negative side of LENS, intended as an
            ablation when combined with `lens_apply_to_positives=True` to isolate the all-positive-group rescue.
            At least one of `lens_apply_to_negatives` or `lens_apply_to_positives` must be `True` when
            `use_lens=True`.
        lens_apply_to_negatives_in_warmup (`bool`, *optional*, defaults to `False`):
            If `True` (requires `use_lens=True` and `lens_apply_to_negatives=True`), keep the negative-LENS term
            active during the `positive_advantage_only` warmup window instead of suppressing it. By default the
            negative-LENS term is gated off whenever `positive_advantage_only` is masking rollouts, which means
            all-negative groups go back to contributing zero gradient during the warmup phase. Enabling this
            flag restores LENS's rescue of all-negative groups across all phases: the calibrated de-mean still
            produces small positive advantages for the least-confidently-wrong rollouts, which survive the
            `positive_advantage_only` mask in `_compute_loss` and yield real gradient.
    ...
    positive_advantage_only: bool = field(
        default=False,
        metadata={
            "help": "If `True`, zero out the per-token loss for any rollout whose advantage is `<= 0`, so only "
            "positive-advantage rollouts contribute gradient. Pilot experiment to test whether negative-advantage "
            "gradients are cancelled by positive ones in the same batch."
        },
    )
    positive_advantage_only_keep_kl: bool = field(
        default=False,
        metadata={
            "help": "Only relevant when `positive_advantage_only=True` and `beta != 0`. If `False` (default), the "
            "positive-advantage mask is applied after the KL term so non-positive-advantage rollouts are fully "
            "dropped (no policy-gradient AND no KL gradient). If `True`, the mask is applied before the KL term "
            "(matching the `off_policy_mask_threshold` precedent), so masked rollouts still contribute KL gradient."
        },
    )
    positive_advantage_only_steps_fraction: float = field(
        default=1.0,
        metadata={
            "help": "Fraction of total training steps during which `positive_advantage_only` is active. Default "
            "`1.0` means always on. Set to e.g. `0.2` to apply positive-advantage masking for the first 20% of "
            "training as a curriculum warmup and then fall back to vanilla GRPO. Only relevant when "
            "`positive_advantage_only=True`."
        },
    )
    negative_advantage_only: bool = field(
        default=False,
        metadata={
            "help": "If `True`, zero out the per-token loss for any rollout whose advantage is `>= 0` during the "
            "final `negative_advantage_only_steps_fraction` of training, so only negative-advantage rollouts "
            "contribute gradient. Mirror of `positive_advantage_only`. Combine with `positive_advantage_only=True` "
            "and `positive_advantage_only_steps_fraction=0.2` for a 3-phase curriculum: positive-warmup → full "
            "GRPO → negative-cooldown."
        },
    )
    negative_advantage_only_steps_fraction: float = field(
        default=0.2,
        metadata={
            "help": "Fraction of total training steps (from the end) during which `negative_advantage_only` is "
            "active. Default `0.2` = final 20% of training. Only relevant when `negative_advantage_only=True`. "
            "Must satisfy `positive_advantage_only_steps_fraction + negative_advantage_only_steps_fraction <= 1.0` "
            "when both flags are enabled."
        },
    )
    negative_advantage_only_keep_kl: bool = field(
        default=False,
        metadata={
            "help": "Only relevant when `negative_advantage_only=True` and `beta != 0`. If `False` (default), the "
            "negative-advantage mask is applied after the KL term so non-negative-advantage rollouts are fully "
            "dropped (no policy-gradient AND no KL gradient). If `True`, the mask is applied before the KL term, "
            "so masked rollouts still contribute KL gradient. Has no effect when `beta=0`."
        },
    )
    linear_advantage_interpolation: bool = field(
        default=False,
        metadata={
            "help": "If `True`, replace the binary positive/negative advantage masks with a per-step linear "
            "interpolation. At step 0 the schedule is `(pos_weight, neg_weight) = (1.0, 0.0)` (positives-only); "
            "at the final step it is `(0.0, 1.0)` (negatives-only); intermediate steps are weighted linearly. "
            "When `positive_advantage_only` / `negative_advantage_only` are set, their `_steps_fraction` windows "
            "clamp the schedule endpoints. Incompatible with `reward_as_advantage`, "
            "`positive_advantage_only_keep_kl`, and `negative_advantage_only_keep_kl`."
        },
    )
    reward_as_advantage: bool = field(
        default=False,
        metadata={
            "help": "If `True`, skip the group-relative baseline and use the (weighted-sum) reward directly as the "
            "advantage. Intended for binary `0/1` reward schemes. When `True`, `num_generations` may be set to `1` "
            "for REINFORCE-style single-rollout training."
        },
    )
    reward_as_advantage_signed: bool = field(
        default=False,
        metadata={
            "help": "If `True` (requires `reward_as_advantage=True`), remap the binary reward via "
            "`advantages = 2 * rewards - 1` so `r=0` rollouts become `-1` instead of `0`. Under vanilla "
            "`reward_as_advantage`, failures contribute zero gradient; the signed variant additionally pushes the "
            "policy away from `r=0` rollouts without going through a group baseline. Foot-gun: combining with "
            "`positive_advantage_only=True` (always-on) silently masks out the new negatives — leave the mask off, "
            "or set `positive_advantage_only_steps_fraction < 1.0`."
        },
    )
    use_lens: bool = field(
        default=False,
        metadata={
            "help": "If `True`, enable LENS (Feng et al., 2025): replace group-relative advantages with "
            "confidence-calibrated rewards so all-negative groups contribute non-zero gradient. Requires binary "
            "0/1 rewards."
        },
    )
    lens_alpha: float = field(
        default=0.25,
        metadata={
            "help": "Down-weight factor on the advantage of rollouts in all-negative groups under LENS. Paper "
            "default: 0.25. No effect when `use_lens=False`."
        },
    )
    lens_apply_to_positives: bool = field(
        default=False,
        metadata={
            "help": "If `True` (requires `use_lens=True`), also discount correct (r=1) rollouts by the same "
            "confidence-calibrated penalty LENS applies to incorrect rollouts: r̃_i = r_i - (1/G) * π̄_old / "
            "(D - π̄_old). Rescues all-positive groups by injecting a differential signal that pushes the policy "
            "toward less-confident correct rollouts. Composes with `positive_advantage_only=True` to run a "
            "positives-only LENS pilot; in that combo, zero-reward rollouts are fully ignored."
        },
    )
    lens_apply_to_negatives: bool = field(
        default=True,
        metadata={
            "help": "If `True` (default, requires `use_lens=True`), apply the LENS confidence-calibrated penalty to "
            "incorrect (r=0) rollouts (the standard paper behavior). Set to `False` to disable the negative side of "
            "LENS — useful as an ablation when combined with `lens_apply_to_positives=True` to isolate the effect of "
            "the all-positive-group rescue. At least one of `lens_apply_to_negatives` or `lens_apply_to_positives` "
            "must be `True` when `use_lens=True`."
        },
    )
    lens_apply_to_negatives_in_warmup: bool = field(
        default=False,
        metadata={
            "help": "If `True` (requires `use_lens=True` and `lens_apply_to_negatives=True`), keep the negative-LENS "
            "term active during the `positive_advantage_only` warmup window instead of suppressing it. By default "
            "the negative-LENS term is gated off whenever `positive_advantage_only` is masking rollouts; this flag "
            "restores LENS's rescue of all-negative groups across all phases (the calibrated de-mean still yields "
            "small positive advantages for the least-confidently-wrong rollouts, which survive the mask)."
        },
    )
    ...
    def __post_init__(self):
        ...
        if self.num_generations < 2 and not self.reward_as_advantage:
            raise ValueError(
                "GRPO requires at least 2 generations per prompt to calculate the advantages. You provided "
                f"{self.num_generations}, which is less than the minimum required. (Set `reward_as_advantage=True` "
                "to allow `num_generations=1` REINFORCE-style training.)"
            )

        if self.reward_as_advantage_signed and not self.reward_as_advantage:
            raise ValueError(
                "`reward_as_advantage_signed=True` has no effect unless `reward_as_advantage=True`."
            )

        if self.positive_advantage_only_keep_kl and not self.positive_advantage_only:
            raise ValueError(
                "`positive_advantage_only_keep_kl=True` has no effect unless `positive_advantage_only=True`. "
                "Either enable `positive_advantage_only` or unset `positive_advantage_only_keep_kl`."
            )

        if self.negative_advantage_only_keep_kl and not self.negative_advantage_only:
            raise ValueError(
                "`negative_advantage_only_keep_kl=True` has no effect unless `negative_advantage_only=True`. "
                "Either enable `negative_advantage_only` or unset `negative_advantage_only_keep_kl`."
            )

        if not 0.0 < self.positive_advantage_only_steps_fraction <= 1.0:
            raise ValueError(
                "`positive_advantage_only_steps_fraction` must be in (0.0, 1.0]; got "
                f"{self.positive_advantage_only_steps_fraction}."
            )

        if not 0.0 < self.negative_advantage_only_steps_fraction <= 1.0:
            raise ValueError(
                "`negative_advantage_only_steps_fraction` must be in (0.0, 1.0]; got "
                f"{self.negative_advantage_only_steps_fraction}."
            )

        if (
            self.positive_advantage_only
            and self.negative_advantage_only
            and self.positive_advantage_only_steps_fraction + self.negative_advantage_only_steps_fraction > 1.0
        ):
            raise ValueError(
                "Positive and negative curriculum windows overlap: "
                f"`positive_advantage_only_steps_fraction` ({self.positive_advantage_only_steps_fraction}) + "
                f"`negative_advantage_only_steps_fraction` ({self.negative_advantage_only_steps_fraction}) > 1.0."
            )

        if self.linear_advantage_interpolation:
            if self.reward_as_advantage:
                raise ValueError(
                    "`linear_advantage_interpolation=True` is incompatible with `reward_as_advantage=True`: "
                    "`reward_as_advantage` produces non-negative advantages, so the negative-weight side of the "
                    "schedule has no effect."
                )
            if self.positive_advantage_only_keep_kl or self.negative_advantage_only_keep_kl:
                raise ValueError(
                    "`linear_advantage_interpolation=True` is incompatible with "
                    "`positive_advantage_only_keep_kl=True` / `negative_advantage_only_keep_kl=True`: the "
                    "continuous interpolation weight is always applied after the KL term."
                )

        if self.use_lens:
            if self.reward_as_advantage:
                raise ValueError("`use_lens=True` is incompatible with `reward_as_advantage=True`.")
            if self.multi_objective_aggregation != "sum_then_normalize":
                raise ValueError(
                    "`use_lens=True` requires `multi_objective_aggregation='sum_then_normalize'`."
                )
            if (
                self.positive_advantage_only
                and not self.lens_apply_to_positives
                and not self.lens_apply_to_negatives_in_warmup
                and self.positive_advantage_only_steps_fraction >= 1.0
            ):
                raise ValueError(
                    "`use_lens=True` with `positive_advantage_only=True` requires "
                    "`positive_advantage_only_steps_fraction < 1.0` (curriculum warmup, so the post-warmup phase "
                    "can emit LENS-calibrated negative gradient), `lens_apply_to_positives=True`, or "
                    "`lens_apply_to_negatives_in_warmup=True`; got "
                    f"`positive_advantage_only_steps_fraction={self.positive_advantage_only_steps_fraction}`."
                )
            if not self.lens_apply_to_negatives and not self.lens_apply_to_positives:
                raise ValueError(
                    "`use_lens=True` requires at least one of `lens_apply_to_negatives` or "
                    "`lens_apply_to_positives` to be `True`; otherwise LENS has no effect."
                )
            if self.lens_apply_to_negatives_in_warmup and not self.lens_apply_to_negatives:
                raise ValueError(
                    "`lens_apply_to_negatives_in_warmup=True` requires `lens_apply_to_negatives=True`; otherwise "
                    "the negative-LENS term is off entirely and the flag has no effect."
                )
        if self.lens_apply_to_positives and not self.use_lens:
            raise ValueError("`lens_apply_to_positives=True` has no effect unless `use_lens=True`.")
        if not self.lens_apply_to_negatives and not self.use_lens:
            raise ValueError("`lens_apply_to_negatives=False` has no effect unless `use_lens=True`.")
        if self.lens_apply_to_negatives_in_warmup and not self.use_lens:
            raise ValueError("`lens_apply_to_negatives_in_warmup=True` has no effect unless `use_lens=True`.")
        if not self.use_lens and self.lens_alpha != 0.25:
            raise ValueError("`lens_alpha` has no effect unless `use_lens=True`.")
