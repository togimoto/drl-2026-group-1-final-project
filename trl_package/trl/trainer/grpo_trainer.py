...
class GRPOTrainer(_BaseTrainer):
    ...
    def __init__(
        ...
        self.positive_advantage_only = args.positive_advantage_only
        self.positive_advantage_only_keep_kl = args.positive_advantage_only_keep_kl
        self.positive_advantage_only_steps_fraction = args.positive_advantage_only_steps_fraction
        self.negative_advantage_only = args.negative_advantage_only
        self.negative_advantage_only_steps_fraction = args.negative_advantage_only_steps_fraction
        self.negative_advantage_only_keep_kl = args.negative_advantage_only_keep_kl
        self.reward_as_advantage = args.reward_as_advantage
        self.reward_as_advantage_signed = args.reward_as_advantage_signed
        self.linear_advantage_interpolation = args.linear_advantage_interpolation
        if self.use_liger_kernel and (
            self.positive_advantage_only
            or self.negative_advantage_only
            or self.reward_as_advantage
            or self.linear_advantage_interpolation
        ):
            raise ValueError(
                "Liger kernel does not support `positive_advantage_only`, `negative_advantage_only`, "
                "`reward_as_advantage`, or `linear_advantage_interpolation` yet."
            )
        self.use_lens = args.use_lens
        self.lens_alpha = args.lens_alpha
        self.lens_apply_to_positives = args.lens_apply_to_positives
        self.lens_apply_to_negatives = args.lens_apply_to_negatives
        self.lens_apply_to_negatives_in_warmup = args.lens_apply_to_negatives_in_warmup
        if self.use_liger_kernel and self.use_lens:
            raise ValueError("Liger kernel does not support `use_lens` yet.")
        # Cumulative counters across training; logged every step like `num_tokens`.
        self._rollouts_used_total = 0
        self._adv_zero_total = 0
        self._adv_positive_total = 0
        self._adv_negative_total = 0
    ...
    def _generate(self, prompts: list):
        ...
        self._metrics[mode]["completions/clipped_count"].append(agg_is_truncated.sum().item())
    ...
    def _generate_and_score_completions(
        ...
        if self.use_lens:
            # LENS (Feng et al., 2025): replace the group-relative advantage with a calibrated reward that assigns
            # non-zero, confidence-dependent penalties to incorrect generations. See https://huggingface.co/papers/2510.08696.
            rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).nansum(dim=1)

            # Length-normalized geometric-mean old-policy sequence probability, per local rollout.
            mask_eff = completion_mask if tool_mask is None else completion_mask * tool_mask
            local_lengths = mask_eff.sum(dim=-1).clamp(min=1).to(old_per_token_logps.dtype)
            local_logp_mean = (old_per_token_logps * mask_eff).sum(dim=-1) / local_lengths
            local_pi_old = local_logp_mean.exp().float()

            # Gather across processes so π_old aligns with the already-gathered rewards (shape [B_global*G]).
            pi_old = gather(local_pi_old)

            # Group reshape and identify all-negative groups.
            rewards_g = rewards.view(-1, num_generations)
            pi_old_g = pi_old.view(-1, num_generations)
            is_neg_group = rewards_g.sum(dim=1) == 0

            # D_imp(q) = ((1/G) Σ r_i / π_old_i)^(-1); undefined for negative groups, where we use the 2·max_π
            # fallback. clamp(min=1e-12) keeps both branches finite; torch.where picks per group.
            ratio = rewards_g / pi_old_g.clamp(min=1e-12)
            D_imp = num_generations / ratio.sum(dim=1).clamp(min=1e-12)
            two_max_pi = 2.0 * pi_old_g.max(dim=1).values
            D = torch.where(is_neg_group, two_max_pi, torch.maximum(D_imp, two_max_pi))
            D_per_sample = D.repeat_interleave(num_generations)

            # Negative LENS (current behavior): r=0 rollouts get r̃_i = -(1/G) * π_old / (D - π_old).
            # Positive LENS (new, gated on `lens_apply_to_positives`): r=1 rollouts get the same discount,
            # so r̃_i = 1 - (1/G) * π_old / (D - π_old). When `positive_advantage_only=True` is also on,
            # we drop the negative-LENS term: those r=0 rollouts get masked downstream anyway, and zeroing
            # their r̃ keeps all-negative groups at advantage = 0 (no leakage via the de-mean + α path).
            # D - π_old > 0 by construction.
            penalty = pi_old / (D_per_sample - pi_old).clamp(min=1e-12)
            # Mirror the `_compute_loss` curriculum gate: when `positive_advantage_only` is active but the
            # warmup window is still open, suppress the negative-LENS term (those r=0 rollouts get masked
            # downstream). Once the window closes, LENS-calibrated negatives flow through normally.
            # `lens_apply_to_negatives_in_warmup=True` bypasses this gate so LENS rescues all-negative groups in
            # all phases (the least-confidently-wrong rollouts pick up positive advantage after the de-mean and
            # survive the loss-level `positive_advantage_only` mask).
            pos_adv_active = self.positive_advantage_only and (
                self.state.global_step < self.state.max_steps * self.positive_advantage_only_steps_fraction
            )
            neg_active = self.lens_apply_to_negatives and (
                self.lens_apply_to_negatives_in_warmup or not pos_adv_active
            )
            pos_active = self.lens_apply_to_positives
            neg_term = (1.0 - rewards) * (penalty / num_generations) if neg_active else torch.zeros_like(rewards)
            pos_term = rewards * (penalty / num_generations) if pos_active else torch.zeros_like(rewards)
            r_tilde = rewards - neg_term - pos_term

            # Group normalization: mixed groups use standard mean+std; negative groups use de-mean only then ×α.
            r_tilde_g = r_tilde.view(-1, num_generations)
            mean_g = r_tilde_g.mean(dim=1).repeat_interleave(num_generations)
            std_g = r_tilde_g.std(dim=1).repeat_interleave(num_generations)
            is_neg_flat = is_neg_group.repeat_interleave(num_generations)
            adv_mixed = (r_tilde - mean_g) / (std_g + 1e-4)
            adv_neg = self.lens_alpha * (r_tilde - mean_g)
            advantages = torch.where(is_neg_flat, adv_neg, adv_mixed)

            std_rewards = std_g
            is_std_zero = torch.isclose(std_rewards, torch.zeros_like(std_rewards))

            # LENS-specific diagnostics.
            self._metrics[mode]["lens/D_q_mean"].append(D.mean().item())
            self._metrics[mode]["lens/D_imp_used_frac"].append(
                ((~is_neg_group) & (D_imp >= two_max_pi)).float().mean().item()
            )
            if neg_active and (rewards == 0).any():
                self._metrics[mode]["lens/penalty_neg_mean"].append(neg_term[rewards == 0].mean().item())
            if pos_active and (rewards == 1).any():
                self._metrics[mode]["lens/discount_pos_mean"].append(pos_term[rewards == 1].mean().item())

        elif self.reward_as_advantage:
            # Skip the group baseline and use the (weighted-sum) reward directly as the advantage. Intended for binary
            # 0/1 reward schemes; with `positive_advantage_only=True`, only reward=1 rollouts contribute gradient.
            # With `reward_as_advantage_signed=True`, remap `r=0` to `-1` so failures also push the policy away.
            rewards = (rewards_per_func * self.reward_weights.to(device).unsqueeze(0)).nansum(dim=1)
            advantages = 2.0 * rewards - 1.0 if self.reward_as_advantage_signed else rewards
            std_rewards = torch.zeros_like(rewards)
            is_std_zero = torch.ones_like(rewards, dtype=torch.bool)

        elif self.multi_objective_aggregation == "sum_then_normalize":
        ...
        # Reward-count diagnostics (binary-reward pilot: rewards ∈ {0, 1}).
        self._metrics[mode]["rewards/num_zero"].append((rewards == 0).sum().item())
        self._metrics[mode]["rewards/num_positive"].append((rewards > 0).sum().item())

        # Per-prompt breakdown (only meaningful when num_generations > 1).
        if num_generations > 1:
            grouped = rewards.view(-1, num_generations)
            num_positive_per_prompt = (grouped > 0).sum(dim=1)
            all_zero = num_positive_per_prompt == 0
            all_one = num_positive_per_prompt == num_generations
            mixed = ~(all_zero | all_one)
            self._metrics[mode]["prompts/frac_all_zero"].append(all_zero.float().mean().item())
            self._metrics[mode]["prompts/frac_all_one"].append(all_one.float().mean().item())
            self._metrics[mode]["prompts/frac_mixed"].append(mixed.float().mean().item())
            # Avg reward across mixed prompts: per-prompt rollout mean, then mean over mixed prompts.
            # Skipped when no mixed prompts this step (wandb shows a gap).
            if mixed.any():
                per_prompt_mean = grouped.float().mean(dim=1)
                self._metrics[mode]["prompts/mixed_avg_reward"].append(per_prompt_mean[mixed].mean().item())

        # Advantage sign breakdown.
        adv_zero = (all_process_advantages == 0).sum().item()
        adv_positive = (all_process_advantages > 0).sum().item()
        adv_negative = (all_process_advantages < 0).sum().item()
        self._metrics[mode]["advantages/num_zero"].append(adv_zero)
        self._metrics[mode]["advantages/num_positive"].append(adv_positive)
        self._metrics[mode]["advantages/num_negative"].append(adv_negative)

        # Rollouts that actually receive gradient this step. Positive-only window: first fraction of training.
        # Negative-only window: final fraction of training. Positive takes priority if both somehow active.
        # When `linear_advantage_interpolation=True`, a rollout contributes if its (pos_weight or neg_weight) for
        # its advantage sign is nonzero; also log the schedule weights so the curriculum is visible in wandb.
        if self.linear_advantage_interpolation:
            pos_w, neg_w = self._advantage_interpolation_weights()
            self._metrics[mode]["schedule/pos_weight"].append(pos_w)
            self._metrics[mode]["schedule/neg_weight"].append(neg_w)
            num_rollouts_used = (
                (adv_positive if pos_w > 0 else 0) + (adv_negative if neg_w > 0 else 0)
            )
        else:
            pos_adv_active = self.positive_advantage_only and (
                self.state.global_step < self.state.max_steps * self.positive_advantage_only_steps_fraction
            )
            neg_adv_active = self.negative_advantage_only and (
                self.state.global_step >= self.state.max_steps * (1.0 - self.negative_advantage_only_steps_fraction)
            )
            if pos_adv_active:
                num_rollouts_used = adv_positive
            elif neg_adv_active:
                num_rollouts_used = adv_negative
            else:
                num_rollouts_used = all_process_advantages.numel()
        self._metrics[mode]["gradients/num_rollouts_used"].append(num_rollouts_used)

        # Running totals — accumulate during training only (mirrors `num_tokens`).
        if mode == "train":
            self._rollouts_used_total += num_rollouts_used
            self._adv_zero_total += adv_zero
            self._adv_positive_total += adv_positive
            self._adv_negative_total += adv_negative
        self._metrics[mode]["gradients/total_rollouts_used"] = [self._rollouts_used_total]
        self._metrics[mode]["advantages/total_zero"] = [self._adv_zero_total]
        self._metrics[mode]["advantages/total_positive"] = [self._adv_positive_total]
        self._metrics[mode]["advantages/total_negative"] = [self._adv_negative_total]
    ...
    def _advantage_interpolation_weights(self) -> tuple[float, float]:
        # Step-dependent (pos_weight, neg_weight) schedule for `linear_advantage_interpolation`.
        # Endpoints: (1.0, 0.0) at step 0, (0.0, 1.0) at the final step. If `positive_advantage_only` /
        # `negative_advantage_only` are set, their `_steps_fraction` windows clamp the schedule so it stays at
        # (1.0, 0.0) for the first `pos_end` of training and (0.0, 1.0) for the last `1 - neg_start` of training.
        step = self.state.global_step
        max_steps = max(self.state.max_steps, 1)
        pos_end = self.positive_advantage_only_steps_fraction if self.positive_advantage_only else 0.0
        neg_start = (1.0 - self.negative_advantage_only_steps_fraction) if self.negative_advantage_only else 1.0
        progress = step / max_steps
        if progress < pos_end:
            return 1.0, 0.0
        if progress >= neg_start:
            return 0.0, 1.0
        span = neg_start - pos_end
        t = (progress - pos_end) / span if span > 0 else 1.0
        return 1.0 - t, t
    ...
    def _compute_loss(self, model, inputs):
        ...
        # Compute the positive-advantage mask once. Apply it after the KL term by default so non-positive-advantage
        # rollouts are fully dropped, or before the KL term if `positive_advantage_only_keep_kl=True` (matches the
        # `off_policy_mask` precedent), which lets masked rollouts still contribute KL gradient. The
        # `positive_advantage_only_steps_fraction` knob gates the entire block on a curriculum window: after the
        # first fraction of training, masking turns off and the loss reverts to vanilla GRPO.
        # When `linear_advantage_interpolation=True`, the binary masks are replaced by a continuous per-step
        # (pos_weight, neg_weight) schedule applied unconditionally after the KL term.
        pos_adv_mask = None
        neg_adv_mask = None
        adv_interp_w = None
        if self.linear_advantage_interpolation:
            pos_w, neg_w = self._advantage_interpolation_weights()
            adv_interp_w = (
                pos_w * (advantages > 0).to(dtype=mask.dtype)
                + neg_w * (advantages < 0).to(dtype=mask.dtype)
            )  # (B, 1) broadcasts over T
        else:
            pos_adv_active = self.positive_advantage_only and (
                self.state.global_step < self.state.max_steps * self.positive_advantage_only_steps_fraction
            )
            # Mirror mask for the negative-only cooldown phase (final fraction of training). Positive takes priority
            # if both windows are somehow active simultaneously (config validator prevents overlap, but guard here too).
            neg_adv_active = self.negative_advantage_only and (
                self.state.global_step >= self.state.max_steps * (1.0 - self.negative_advantage_only_steps_fraction)
            ) and not pos_adv_active
            if pos_adv_active:
                pos_adv_mask = (advantages > 0).to(dtype=mask.dtype)  # (B, 1) broadcasts over T
                if self.positive_advantage_only_keep_kl:
                    per_token_loss = per_token_loss * pos_adv_mask
            elif neg_adv_active:
                neg_adv_mask = (advantages < 0).to(dtype=mask.dtype)  # (B, 1) broadcasts over T
                if self.negative_advantage_only_keep_kl:
                    per_token_loss = per_token_loss * neg_adv_mask
        ...
        if adv_interp_w is not None:
            per_token_loss = per_token_loss * adv_interp_w
        if pos_adv_mask is not None and not self.positive_advantage_only_keep_kl:
            per_token_loss = per_token_loss * pos_adv_mask
        if neg_adv_mask is not None and not self.negative_advantage_only_keep_kl:
            per_token_loss = per_token_loss * neg_adv_mask
